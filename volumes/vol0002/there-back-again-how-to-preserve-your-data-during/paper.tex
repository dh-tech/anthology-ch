...
...
\end{minted}
 \captionof{listing}{XML file fragment with entity count (top), and Drupal entity count command (bottom) for entities of type PLACE.}
...
300
\end{minted}
 \captionof{listing}{\texttt{drush} command to extract the count of Drupal entities by type.}
\label{code:ismi-cnt}
\end{listing}
...
...                        
        ...
        links.add(link)
\end{minted}
  \captionof{listing}{Python script fragment showing a loop processing all \texttt{a} tags in an HTML document and adding a processed item.}
...
\end{minted}
  \captionof{listing}{Python script fragment with a highlighted set of processed items and set math to determine unprocessed items.}
...
\end{minted}
  \captionof{listing}{Command line options for the Python model comparison tool \texttt{compare\_models}, including options to show more or less detailed differences or to ignore specific item, attribute, or relation types.}
